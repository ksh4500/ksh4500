var Flame=function(a,w,h,posX,posY){
	/*현실의 모든 사물은 객체지향언어에서 클래스로 표현되고, 해당사물이 보유한 상태는 변수인 속성으로 표현하며, 해당 사물이 보유한 동작은 함수인 메서드로 표현한다.
	하지만, 변수와 함수가 객체의 소유가 될때는 명칭이 바뀐다.
	변수: 객체가 보유한 상태를 표현한다고 해서 속성(property)
	함수: 객체가 보유한 동작방식을 표현한다고 해서 매서드(method)
	*/
	/*가로, 세로 크기, 위치*/
	this.width=w;
	this.height=h;
	...
	//이 객체가 태어날때(메모리에 올라올때) 하고싶은 초기작업에 사용될 메서드 정의
	this.init=function(){
