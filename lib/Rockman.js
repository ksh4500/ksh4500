var Rockman=function(stage,width,height,x,y,src){
	...
	this.move=function(){
		...
		//중력에 의해 다시 물체가 떨어지기 시작하는 시점에는 다시 falling을 true로 전환
		if(this.velY>0){
			this.jumping=false;
			this.falling=true;//주인공이 떨어지고 있음으로 전환
		}
		...
		for(var i=0; i<blockArray.length; i++){
			...
			if(result && this.falling){
				this.velY=0;//밟으면 떨어지지 않게 velY값을 0으로 논다
				console.log("벽돌 밟았어!!");
